...
Note   :   If either the salary is 0 or negative  (or) if the appraisal rating is not
in the range 1 to 5 (inclusive), then the output should be “Invalid Input”.
Sample Input 1 :
Enter the salary
8000
...
public class SalaryIncrement {
    float result(float rating,float salary)
    {
        if(rating>=1 && rating<=3)
        {
            salary+=salary*((float)10/100);
        }
        if(rating>=3.1 && rating<=4)
        {
            salary+=salary*((float)25/100);
        }
        if(rating>=4.1 && rating<=5)
        {
            salary+=salary*((float)30/100);
        }
        return salary;
    }
    public static void main(String[] args) {
       ...
       System.out.println( obj.result(rating, salary));
    }    
}
