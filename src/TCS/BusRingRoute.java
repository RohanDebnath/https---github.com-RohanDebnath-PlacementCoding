...
// Not the exact Thing wanted
import java.util.HashMap;
import java.util.Map;
import java.util.Scanner;

public class BusRingRoute {
    static int result(HashMap<String, Integer> m, String sre, String destination) {
        int sreVal = m.get(sre);
        int desVal = m.get(destination);
        int val = Math.abs(desVal - sreVal);
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Provide Key and Value Pair");
        for (int i = 0; i < n; i++) {
            key = sc.nextLine();
            val = sc.nextInt();
        ...
        System.out.println(result(m, Sre, destination));
    }
}
